...
class _HobbyPageState extends State<HobbyPage> {
  ...
  return users
    ...
    return snapshot.docs.map((doc) {
      final hobbyData = doc.data() as Map<String, dynamic>;
      
      // If your level/xp is nested inside an 'info' map in Firestore:
      final info = hobbyData['info'] as Map<String, dynamic>? ?? {};
      final mergedData = {...info, ...hobbyData};

      // Fix: Pass doc.id directly as the String hobbyName
      return Hobby.fromMap(doc.id, mergedData);
    }).toList();
  });
}
